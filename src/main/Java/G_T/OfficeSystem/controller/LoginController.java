...
public class LoginController {
    ...
    @RequestMapping (value = "/Login", params = { "userId",
            "password" }, method = RequestMethod.POST, produces = "text/plain;charset=utf-8")
    public @ResponseBody String Login (@RequestParam (value = "userId") String userId,
            @RequestParam (value = "password") String password, @RequestParam (value = "email") String email,
            Model model) {
        /*
         * if (userId == "" || password == "") {
         * model.addAttribute("error","ユーザーIDまたはパスワードが未入力です。"); return "Login"; }
         */
        ...
        if (count == 0) {
            return "ユーザーID、パスワードまたはメールアドレスが存在しません";
        }
