...
public class UserManageModel {
    ...
    public void UserManage (Integer type) {
        setAllUserList (userInfoModelDAO.UserManage (type));
    ...
    public void SortAll (String sortColumn, String sortOrder) {
        Collections.sort (this.allUserList, new Comparator<UserInfoModel> () {
            public int compare (UserInfoModel u1, UserInfoModel u2) {
                ...
                } else if (sortColumn.equals ("生年月日")) {
                    return invertFlag * (u1.getBirthday ().compareTo (u2.getBirthday ()) > 0 ? 1 : -1);
                } else if (sortColumn.equals ("メール")) {
                    return invertFlag * (u1.getAddress ().compareTo (u2.getAddress ()) > 0 ? 1 : -1);
                } else if (sortColumn.equals ("所属")) {
                    return invertFlag * (u1.getAffiliation ().compareTo (u2.getAffiliation ()) > 0 ? 1 : -1);
                } else if (sortColumn.equals ("役職")) {
                    return invertFlag * (u1.getPosition ().compareTo (u2.getPosition ()) > 0 ? 1 : -1);
                } else {
                    return invertFlag * 1;
                }
            }
